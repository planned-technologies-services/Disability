...
(function () {
    ...
    _app.prototype = {
                ...
                if (editFields.length) {
                    ...
                    $(editFields).each(function () {
                        var f = this,
                            ...
                            qcb = { name: f.Name + '_BatchEdit', type: 'bool', value: false, required: !_app.touch, text: f.HeaderText || f.Label, items: { style: 'CheckBox' }, options: { lookup: { autoAdvance: true } } },
                            q = {
        ...
        hasDetails: function () {
            ...
            return !!result && !that._doneCallback;
        },
                ...
                if (field.OnDemand) this._renderOnDemandItem(sb, field, row, isSelected, isForm);
                    ...
                    if (fieldItems.length == 0) {
                        ...
                        if (v && field.TextMode == 3)
                            v = v.replace(/\r?\n/g, '<br/>');
                        s = String.isBlank(v) ? (isForm ? resourcesData.NullValueInForms : resourcesData.NullValue) : (this._allFields[field.AliasIndex].format(v));
                    }
                    else if (isCheckBoxList) {
                        lov = v ? v.split(',') : [];
                        var fi = true;
            ...
            for (i = 0; i < currentFilter.length; i++) {
                ...
                if (!bannerRendered) {
                    if (includeBanner != false)
                        sb.appendFormat('<a href="javascript:" onclick="$find(\'{0}\').clearFilter(true);return false" class="Close" tabindex="{3}" title="{2}">&nbsp;</a><span class="Details"><span class="Information">&nbsp;</span>{1} ', this.get_id(), Web.DataViewResources.InfoBar.FilterApplied, Web.DataViewResources.ModalPopup.Close, $nextTabIndex());
                    bannerRendered = true;
                }
        ...
        _onGetPageComplete: function (result, context) {
            ...
            if (that._pageIndex < 0 || positionChanged) {
                if (this._pageIndex == -1 || positionChanged) {
                    ...
                    that._disposeFields();
                    that._expressions = result.Expressions;
                    that._detachBehaviors();
                    that._allFields = result.Fields;
                    ...
                    for (i = 0; i < result.Fields.length; i++) {
                        ...
                        if (!String.isBlank(field.SearchOptions) && !(__tf != 4)) {
                            ...
                            if (field.SearchOptions && _app.touch && field.OriginalIndex != field.Index)
                                result.Fields[field.OriginalIndex].SearchOptions = field.SearchOptions;
                        }
                        else
                        ...
                        if (field.Type == 'Boolean' && field.Items.length == 0) {
                            ...
                            if (!itemsStyle)
                                itemsStyle = field.ItemsStyle = Web.DataViewResources.Data.BooleanDefaultStyle;
                        }
                        if (field.Items && field.Items.length > 0 && (field.AllowNulls && !field.ItemsTargetController || itemsStyle == 'DropDownList' && !itemsDataController) && !String.isNullOrEmpty(field.Items[0][0]) && itemsStyle != 'CheckBoxList' && !field.tagged('lookup-null-value-none'))
            ...
            var rules = new _businessRules(this);
        ...
        _populateDynamicLookups: function (result) {
            ...
            if ($app.mobile)
                ...
                $app.input.execute({ dataView: that, values: result.Values, populateDynamicLookups: false });
            else {
        ...
        _busy: function (isBusy) {
            ...
            if (isBusy && !interval) {
                ...
                var $busy = that._$busy = $(String.format('<div class="dataview-busy-indicator{0}"></div>', atWindowTop ? ' modal' : ''))
                    .css({ position: 'absolute', width: w, 'z-index': atWindowTop ? (mobile ? 5000 : 3000000) : 0 }),
                    x = -w,
    ...
    */
    ...
    _window._field_trim = function (v) {
        var that = this;
        if (that.Type == 'String' && v != null && v.length > resourcesData.MaxReadOnlyStringLen && !(that.TextMode == 3 || that.TextMode == 2))
            v = v.substring(0, resourcesData.MaxReadOnlyStringLen) + '...';
    ...
    Date.tryParseFuzzyDateTime = function (s, dataFormatString) {
        ...
        if (m) {
            ...
            var dtf = Sys.CultureInfo.CurrentCulture.dateTimeFormat;
            if (!String.isNullOrEmpty(dtf.AMDesignator) && (m[2].toLowerCase() == dtf.AMDesignator.toLowerCase() || m[2].toLowerCase() == dtf.PMDesignator.toLowerCase()))
    ...
    */
    ...
    _app.survey = function (method, options) {
        ...
        function toUrl(name) {
            return $app.find(options.parent).get_baseUrl() + (name.match(/\//) ? name : ('/js/surveys/') + name);
        }
        ...
        function createRule(list, funcName, func, commandName, commandArgument, phase, argument) {
            var //s = typeof func === 'string' ? ('function(){var e =$.Event("' + func + '",{rules:this,dataView:this.dataView()});$(document).trigger(e);' + (commandName == 'Calculate' ? '' : 'if(e.isDefaultPrevented())') + 'this.preventDefault();}') : func.toString(),
                s = 'function(){var r=this,dv=r.dataView(),s=dv.survey(),e=$.Event("' + (typeof func === 'string' ? func : funcName) +
        ...
        function refresh(callback) {
            ...
            options.compiled = function (result) {
                ...
                // replace layout
                var newForm = touch.createLayout(dataView, touch.calcWidth(form.parent()));
                newForm = newForm.insertAfter(form);
                touch.prepareLayout(dataView, result.NewRow, newForm);
                form.remove();
